module top_module(
    ...
    //to check odd parity, the XOR of the entire data along with the parity bit must be 1
    //all we need to do is add an extra state to receive the parity bit
    ...
    ,sf = 4'd10         //state sf(set finish) is the finish state when all 8 'in' receiving states(s0-s8) are finished and parity bit is also taken (ps) and finally in = 1 (stop bit) is received
    ,nf = 4'd11         //state nf(not found) is when all 8 bits and parity bit are received but no stop bit is received
    ,ps = 4'd12;        //added an extra state for receiving parity bit
    ...
    always@(*)begin
        case(cs)
            ...
            s8: ns <= ps;  //once in state 8 we go to parity state and receive in(parity bit)
            ps: ns <= in?sf:nf; //once in state parity state we check for stop bit(in =1), if stop bit is received right after we go to finished state, else we go to not found state
    ...
    always@(*) begin
        case(cs)
            ...
            s8: val[8] = in;   //in state 8 the last bit we receive will be parity bit
            ps: val = val;     //in parity state now the next bit would be stop bit and we don't need that so val stays val
    ...
    assign done = (cs == sf)&&(^val);   //done is high when we are in finish state and Odd parity satisfies
    assign out_byte = (done)?val[7:0]:8'b0;  //out_byte gets val only when we are in finish state, else does not.
